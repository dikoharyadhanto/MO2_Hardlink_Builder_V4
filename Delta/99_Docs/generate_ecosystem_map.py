#!/usr/bin/env python3
"""
Generate Delta Ecosystem Workflow Diagram

This script generates the ECOSYSTEM_WORKFLOW.md file with a Mermaid diagram
that shows the complete Delta project lifecycle aligned with DELTA_PROTOCOL.md.

The workflow includes:
- Director Intent (DI) input
- Strategy layer (GMN) STRAT document creation
- Strategy approval (GPT, PPX) - parallel tracks
- Planning layer (ANT) work formulation (WO, STR)
- Implementation layer (CDC) code execution (IMPL, WALK)
- Implementation approval (ANT)
- Final QA (GPT, PPX)
- Director manual testing (optional)
...
def generate_mermaid():
    """Generate the Mermaid diagram for Delta ecosystem workflow."""
    ...
    subgraph SUPPORT["SUPPORTING ARTIFACTS"]
        ...
        KNOW["06_Knowledge_Repo/<br/>NLM Knowledge Modules"]
    ...
    # Markdown content
    markdown_header = """# Delta Ecosystem Workflow
...
├── 00_Global_Rules/      ← Phase 2-8 agent rule definitions (ANT, CDC, GMN)
...
├── 06_Knowledge_Repo/    ← Ecosystem knowledge modules (NLM)
...
""" % datetime.now().strftime("%Y-%m-%d")
    ...
    # Combine everything
    full_content = markdown_header + mermaid_content + "\n```\n" + markdown_phases

    # Write to file in 99_Docs folder
    script_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(script_dir, "ECOSYSTEM_WORKFLOW.md")

    with open(file_path, "w", encoding="utf-8") as f:
        f.write(full_content)

    print(f"* Ecosystem workflow map generated: {file_path}")
    print(f"* Workflow aligns with DELTA_PROTOCOL.md")
    print(f"* 10 phases documented with role assignments")
    print(f"* Document versioning tiers explained")
